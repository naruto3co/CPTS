...
```
naruto3co@htb[/htb]$ fping -asgq 172.16.5.0/23
...
```
naruto3co@htb[/htb]$ sudo git clone https://github.com/ropnop/kerbrute.git
```
Typing make help will show us the compiling options available.

## Listing Compiling Options
```
naruto3co@htb[/htb]$ make help
help:            Show this help.
...
```
naruto3co@htb[/htb]$ sudo make all
...
```
naruto3co@htb[/htb]$ ./kerbrute_linux_amd64 
...
## Moving the Binary
```
naruto3co@htb[/htb]$ sudo mv kerbrute_linux_amd64 /usr/local/bin/kerbrute
```
...
```
naruto3co@htb[/htb]$ kerbrute userenum -d INLANEFREIGHT.LOCAL --dc 172.16.5.5 jsmith.txt -o valid_ad_users
