...
## Mimikatz dùng cho việc gì?
Dump ticket đang tồn tại
Pass-the-Ticket
Lateral movement
Post-exploitation
📌 Nó không sinh ra Kerberoast hash chuẩn để crack password service account.

Mimikatz → xuất ticket → base64 → .kirbi → john/hashcat

### Cân nhắc : 
Mimikatz in ticket ra dạng Base64
Nhưng tool crack không ăn Base64
Nó chỉ ăn file .kirbi nhị phân
👉 Đoạn tài liệu này chỉ dạy bạn chuyển đổi qua lại cho đúng định dạng.


### Cân nhắc 1
Mimikatz cho ta được file .kirbi đã bị encode sang Base64
Dùng lệnh này vì Base64 output từ Mimikatz thường bị xuống dòng, bị ngắt đoạn
```
...
Đang làm gì?
encoded_file = file chứa 1 dòng Base64
base64 -d = decode Base64
-> sqldev.kirbi = ghi ra file nhị phân

👉 Kết quả:
Bạn tái tạo lại file Kerberos ticket thật
File này giống y hệt ticket ban đầu trong RAM
📌 .kirbi không phải text, mà là binary ticket

### Cân nhắc 3
Vì sao cần kirbi2john.py
Hashcat KHÔNG đọc được .kirbi trực tiếp.

👉 Phải có bước trung gian:
```
.kirbi  →  john format  →  hashcat format
```
...
👉 Script này:
đọc cấu trúc Kerberos ticket
trích xuất phần có thể crack
xuất ra file crack_file
📌 Lúc này mới bắt đầu giống Kerberoasting

Vì:
Output của kirbi2john.py
chưa đúng chuẩn Hashcat

cần:
xóa prefix thừa
chỉnh lại format $krb5tgs$23$...
👉 Đây là lý do tài liệu nói “modify a bit”.



##  Nếu họ dùng aes256_hmac để mã hoá thông tin thì sao ? 
| Thứ bạn có                          | Có crack được không |
| ----------------------------------- | ------------------- |
| `.kirbi` dump từ RAM (Mimikatz)     | ❌ Gần như không     |
| Kerberoast hash xin trực tiếp từ DC | ✅ Có                |
| Hashcat mode cho AES256 TGS         | ✅ `-m 19700`        |
👉 Vấn đề không nằm ở AES, mà nằm ở cách bạn lấy dữ liệu.



# làm bài tập 

Mở mimikatz quyền admin
```
...
# Kerberoasting – Tổng hợp các cách theo tài liệu

> Mục tiêu: **Crack password của service account có SPN** (ví dụ: `svc_vmwaresso` với SPN `vmware/inlanefreight.local`).

---

## 1. Những gì KHÔNG dùng để crack password service account

### 1.1 Mimikatz – Dump ticket từ RAM
...
* Lệnh thường gặp:
  ...
  mimikatz # kerberos::list /export
  ```
* Kết quả: file `.kirbi` (ticket Kerberos đang tồn tại trong RAM)
* Đặc điểm:

  * Ticket thường là của **user đang login** (ví dụ `htb-student`)
  * Thường dùng **AES256 (etype 18)**
* ❌ Không phù hợp để crack password service account
* ✅ Dùng cho:
...
### 1.2 `kirbi2john.py` với ticket dump từ Mimikatz

* Chỉ hoạt động tốt với **RC4 (etype 23)**
* Không xử lý được AES256 từ ticket dump kiểu này
* ❌ Không ra hash → không crack được

---

## 2. Cách ĐÚNG để crack password service account (Kerberoasting)

> Nguyên tắc: **Xin TGS trực tiếp từ Domain Controller cho SPN** → lấy hash → crack offline

---

## 3. Cách 1 – Kerberoasting bằng Impacket (Linux) ⭐ Khuyến nghị

### 3.1 Điều kiện

* Có domain credential hợp lệ (user thường cũng được)
* Biết domain, DC IP, SPN hoặc service account

### 3.2 Lệnh Kerberoast

```bash
GetUserSPNs.py INLANEFREIGHT.LOCAL/sqldev:database! \
-dc-ip 172.16.5.5 -request-user svc_vmwaresso
```
...
* Thu được hash dạng:
  ...
  ```
  $krb5tgs$18$*svc_vmwaresso$INLANEFREIGHT.LOCAL$vmware/inlanefreight.local*$...
  ```
* `18` = AES256

### 3.4 Crack bằng Hashcat

```bash
hashcat -m 19700 svc_vmwaresso.hash rockyou.txt
```

---

## 4. Cách 2 – Kerberoasting bằng Rubeus (Windows)

### 4.1 Kerberoast theo user

```powershell
Rubeus.exe kerberoast /user:svc_vmwaresso
```

### 4.2 Kerberoast theo SPN

```powershell
Rubeus.exe kerberoast /spn:vmware/inlanefreight.local
```

### 4.3 Kết quả

* Rubeus xuất hash **sẵn cho Hashcat**
* Hỗ trợ cả RC4 và AES

---

## 5. So sánh nhanh các cách

| Cách                  | Xin TGS từ DC | Crack được password | Ghi chú                     |
| --------------------- | ------------- | ------------------- | --------------------------- |
| Mimikatz dump ticket  | ❌             | ❌                   | Chỉ lấy ticket đang tồn tại |
| kirbi2john + Mimikatz | ❌             | ❌ (AES)             | Tool hạn chế                |
| Impacket Kerberoast   | ✅             | ✅                   | Chuẩn, ổn định              |
| Rubeus Kerberoast     | ✅             | ✅                   | Thuận tiện trên Windows     |

---

## 6. Kết luận (rất quan trọng)

> **Muốn crack password service account → BẮT BUỘC Kerberoasting (Impacket hoặc Rubeus).**

* AES256 **không phải vấn đề**
* Vấn đề là **lấy đúng loại dữ liệu (TGS xin từ DC)**
...
* `.kirbi` dump từ RAM ≠ Kerberoast hash
* Impacket / Rubeus = con đường thẳng nhất để qua lab
