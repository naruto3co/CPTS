...
( mỗi phần trong này chắc phải làm 1 chương riêng để chuyên sâu luôn ý ) 
👉 Ý nghĩa: Tìm các phân vùng ổ đĩa (drive) chưa được gắn vào hệ thống — vì trong đó có thể chứa dữ liệu nhạy cảm giúp leo thang quyền.
Khi bạn khai thác một máy Linux, đặc biệt là máy ảo hoặc server thật, đôi khi có:
  Ổ đĩa phụ chưa được mount
  Phân vùng chứa backup, old home directory, shadow file cũ, hoặc key, config, database…
  Phân vùng cũ của người quản trị (admin)
  Ổ USB/SSD từng cắm nhưng chưa mount
Nếu bạn mount được các phân vùng này → có thể tìm:
  Mật khẩu plaintext
  SSH private key
  File cấu hình chứa DB creds
  Scripts chạy cron với quyền root
  Backup chứa /etc/shadow
  Dữ liệu user khác (root / admin)
➡️ Mount ổ phụ = mở thêm một “cánh cửa” vào hệ thống để tìm thông tin nhạy cảm.
...
Giải thích:
sda = ổ chính
sda1 = phân vùng được mount ở /
sda2, sda5 = phân vùng khác
sr0 = ổ CD-ROM (ISO gắn khi tạo máy ảo)
➡️ Nếu bạn thấy phân vùng không có MOUNTPOINT → bạn có thể mount để tìm bí mật:
```bash
mkdir /mnt/extra
mount /dev/sda2 /mnt/extra
ls /mnt/extra
```
...
SETUID và SETGID Permissions
👉 Ý nghĩa: Tập tin có SUID/SGID cho phép bạn chạy nó với quyền của chủ sở hữu — thường là root.

Nếu binary có SUID root → chạy với quyền root, dù bạn chỉ là user bình thường.

Nhiều binary SUID có thể bị khai thác:
  /usr/bin/find
  /usr/bin/python
  /usr/bin/perl
  /usr/bin/env
  /usr/bin/bash
  v.v…
Ví dụ: nếu find có SUID:
```bash
find . -exec /bin/sh -p \; -quit
```
→ bạn có root shell ngay lập tức.
➡️ Tìm SUID là bước quan trọng nhất trong PrivEsc.
...
/run/lock
```
👉 Ý nghĩa: Thư mục mà user hiện tại có quyền ghi (write) — có thể dùng để tải công cụ, viết script độc hại, hoặc hijack cronjob.
Tại sao quan trọng?
Bạn có thể upload shell, công cụ (linpeas, pspy…) vào thư mục writeable.
Nếu có thư mục writeable liên quan đến cron job, bạn có thể sửa file để thực thi code với quyền root.
Dùng nó để lưu payload, reverse-shell, hoặc script rootkit.

Lệnh tìm tất cả thư mục writeable:
```bash
find / -path /proc -prune -o -type d -perm -o+w 2>/dev/null
```
Kết quả :
```bash
/dmz-backups        <- rất đáng nghi, backup = mật khẩu / shadow / config
/tmp                <- writeable-by-everyone
/var/tmp            <- cũng writeable
/var/crash
/tmp/VMwareDnD
/tmp/.X11-unix
```
...
Giải thích nhanh:
/tmp, /var/tmp = luôn writeable
/dmz-backups = mục cực kỳ quan trọng, thường chứa:
backup database
file zip chứa mật khẩu
old configs
old home directories
➡️ Một thư mục kiểu /dmz-backups gần như luôn dẫn đến Privilege Escalation.
